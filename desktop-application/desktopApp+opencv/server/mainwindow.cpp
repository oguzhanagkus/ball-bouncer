    ...
    , ui(new Ui::MainWindow)
{
    ...
    //change on button thread
    //remove comment for mOpencv_videoCapture
  /*  mOpencv_videoCapture = new videoCapture(this);
    connect(mOpencv_videoCapture,&videoCapture::newPixmapCaptured,this
    ,[&](){
        cameraImage->setPixmap(mOpencv_videoCapture->pixmap().scaled(500,500));

    });


    connect(mOpencv_videoCapture,&videoCapture::dataFromImageProcessing,this
    ,[&](){

         x_img = (double)mOpencv_videoCapture->x;
         y_img = (double)mOpencv_videoCapture->y;
         height_img = (double)mOpencv_videoCapture->height;
         addPoints(x_img,y_img,height_img);
    });*/

   //remove comment for mOpencv_colorTracker
    mOpencv_colorTracker = new colorTracker(this);
    connect(mOpencv_colorTracker,&colorTracker::newPixmapCaptured,this
    ,[&](){
        cameraImage->setPixmap(mOpencv_colorTracker->pixmap().scaled(500,500));
    ...
    ,[&](){
         ...
         x_img = (double)mOpencv_colorTracker->x;
         y_img = (double)mOpencv_colorTracker->y;
         height_img = (double)mOpencv_colorTracker->height;

        // (x,y,z)-> sağsol,yukarıaşağı,ilerigeri
         addPoints(x_img,height_img,y_img);
         sendDatagram();
    });
...
/*
    Updating the graphs after new point is added.
*/
...
void MainWindow::addPoints(double x, double y, double z)
{
      ...
      for(int i=0;i<pointCount;i++){
          ...
          *data << QVector3D(qv_x_pre.at(i), qv_y_pre.at(i),qv_z_pre.at(i));
...
void MainWindow::set3dGraph()
{
    ...
    graph->axisX()->setRange(-30,30);
    graph->axisY()->setRange(15,30);
...
void MainWindow::set2dGraphs()
{
    ...
    graph_xy->xAxis->setRange(0,30);
    graph_xy->yAxis->setRange(0,30);
   ...
   // ui->plot_x_y->setBackground(QBrush(gradient));
    ...
    //x-z graph
    //set range
    graph_xz->xAxis->setRange(0,30);
    graph_xz->yAxis->setRange(0,30);
...
void MainWindow::on_button2d_clicked()
{
    if(flag3d == 1 ) {
        ...
        delete  ui->layout->itemAt(0)->widget();
       // ui->layout->removeWidget(cameraContainer);
        set2dGraphs();
        ui->layout->addWidget(graph_xy);
        ui->layout->addWidget(graph_xz);
        flag3d = 0;
...
void MainWindow::on_opencvButton_clicked()
{
 ...
 //   mOpencv_videoCapture->start(QThread::HighestPriority);
    mOpencv_colorTracker->start(QThread::HighestPriority);
}
...
void MainWindow::on_cameraButton_clicked()
{
    if(flag3d == 0){
        ...
        ui->layout->removeWidget(graph_xz);
        flag3d= 4;
        ui->layout->addWidget(cameraImage);
    }else if(flag3d == 1){
        container->close();
        ui->layout->removeWidget(container);
        flag3d= 4;
...
{
    ui->label1->setText(QString::number( mOpencv_videoCapture->x));
}
