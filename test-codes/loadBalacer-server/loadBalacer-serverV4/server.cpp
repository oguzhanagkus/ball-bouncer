...
//int cornerCenterX[12] = {240,250,270,305,340,355,375,360,345,310,280,260};
//int cornerCenterY[12] = {230,270,300,295,290,270,230,190,175,160,160,180};
//int cornerCenterX[NUMBERPOINT] = {215,230,235,280,340,375,380,365,350,295,260,215};
//int cornerCenterY[NUMBERPOINT] = {215,270,290,300,300,270,210,170,130,130,130,165};
//int cornerCenterX[NUMBERPOINT] = {415,320,245};
//int cornerCenterY[NUMBERPOINT] = {330,230,160};
		...
		{	
    		...
    		if (selection == 1 )
    		{
				...
				if (pidServoY->getOutput() < 0)
					a->motor1 = - pidServoY->getOutput();
				else
					a->motor3 = pidServoY->getOutput();

				if (pidServoX->getOutput() < 0)
					a->motor2 = - pidServoX->getOutput();
				else
					a->motor4 = pidServoX->getOutput();
				...
				usleep(sleepCalculation(lastS1,lastS2,lastS3,lastS4,temp.angle_0,temp.angle_1,temp.angle_2,temp.angle_3));
    			...
    			int durationSecond = duration_cast<milliseconds>(end - start).count();
    			cout<<durationSecond<<endl;
				...
				if (indexBounce == 1)
				{
					temp.angle_0 = (uint8_t)a->motor1+5;//+ BOUNCERUP;
					temp.angle_1 = (uint8_t)a->motor2 ;//+ BOUNCERUP ;
					temp.angle_2 = (uint8_t)a->motor3 ;//+ BOUNCERUP;
					temp.angle_3 = (uint8_t)a->motor4+10;// + BOUNCERUP; //+ BOUNCERUP;
					indexBounce = 0;
				...
				write(port, &temp, sizeof(temp));
				tcflush(port, TCIFLUSH);
				int sleepp = sleepCalculation(lastS1,lastS2,lastS3,lastS4,temp.angle_0,temp.angle_1,temp.angle_2,temp.angle_3);
				...
				printf("%d-------------------------------------\n",sleepp );
				usleep(sleepp);
				//if ((abs(cornerCenterX[indexPoint%NUMBERPOINT]-posX)<120) && (abs(cornerCenterY[indexPoint%NUMBERPOINT]-posY)<120))
				//{
...
int sleepCalculation(int lastS1,int lastS2,int lastS3,int lastS4,
	...
	cout<<"lastS1 "<<lastS1<<" --- "<<"cur"<<cur1<<endl;
	cout<<"lastS2 "<<lastS2<<" --- "<<"cur2"<<cur2<<endl;
	cout<<"lastS3 "<<lastS3<<" --- "<<"cur3"<<cur3<<endl;
	cout<<"lastS4 "<<lastS4<<" --- "<<"cur4"<<cur4<<endl;
	printf("LAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA %d\n",max);
