...
int main(int argc, char** argv)
{
	...
	cin>>selection;
	int cornerCenterX = 320;
	int cornerCenterY = 360;
	int indexPoint = 0;
	while (true)
	{
		Mat imgOriginal;
		...
		{	
    		...
    		if (selection == 1 )
    		{
    			...
    			pidServoX->compute( cornerCenterX - posX );
				pidServoY->compute( cornerCenterY - posY );
				...
				if (indexPoint == 0 && (abs(cornerCenterX-posX)<20) && (abs(cornerCenterY-posY)<20) )
				{
					indexPoint = 1;
					cornerCenterX = 480;
					cornerCenterY = 240;
					...
					pidServoY = new PID( "pidServoX_config" );
					if ( pidServoY->fail() )
					{
							cerr << pidServoY->getErrorStr() << endl;
							exit( EXIT_FAILURE );
					}
				}else if (indexPoint == 1 && (abs(cornerCenterX-posX)<20) && (abs(cornerCenterY-posY)<20))
				{
					indexPoint = 2;
					cornerCenterX = 320;
					cornerCenterY = 120;
				...
				}else if (indexPoint == 2 && (abs(cornerCenterX-posX)<20) && (abs(cornerCenterY-posY)<20))
				{
					indexPoint = 3;
					cornerCenterX = 160;
					cornerCenterY = 240;
				...
				}else if(indexPoint == 3 && (abs(cornerCenterX-posX)<20) && (abs(cornerCenterY-posY)<20)){
					indexPoint = 0;
					cornerCenterX = 320;
					cornerCenterY = 360;
					pidServoX = new PID( "pidServoX_config" );
